...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['comprobanteEnvio']) && $_FILES['comprobanteEnvio']['error'] == UPLOAD_ERR_OK) {
        $target_dir = "php/uploads/";  // Directorio donde se guardará el archivo
        ...
        if (move_uploaded_file($_FILES["comprobanteEnvio"]["tmp_name"], $target_file)) {
            $comprobanteEnvioRuta = "uploads/" . basename($_FILES["comprobanteEnvio"]["name"]);  // Guardar la ruta correcta en la base de datos
        } else {
